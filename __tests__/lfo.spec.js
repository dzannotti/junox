...
describe('LFO', () => {
  ...
  describe('delay', () => {
    it('should envelope delay', () => {
      const lfo = new LFO({ sampleRate: 100, delay: 2, frequency: 1 })
      const out = Array(200)
        .fill(0)
        .map(() => lfo.delayEnv())
    ...
    it('should have an attack slope afterwards', () => {
      ...
      lfo.setDelay(0)
      expect(lfo.attackRate * sampleRate).toBeLessThan(0.02)
      lfo.setDelay(0.0639)
      expect(lfo.attackRate * sampleRate).toBeCloseTo(0.028, 2) // should be (0.053, 2)
      lfo.setDelay(0.85)
      expect(lfo.attackRate * sampleRate).toBeCloseTo(0.211, 1) // should be (0.188, 2)
      lfo.setDelay(1.2)
      expect(lfo.attackRate * sampleRate).toBeCloseTo(0.325, 2) // should be (0.348, 2)
      lfo.setDelay(2.786)
      expect(lfo.attackRate * sampleRate).toBeGreaterThan(1, 2)
    })
  })
})
